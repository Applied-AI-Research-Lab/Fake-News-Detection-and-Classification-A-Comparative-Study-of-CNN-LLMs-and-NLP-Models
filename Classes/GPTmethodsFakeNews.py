            ...
            data.append(
                {
                    "messages": [
                        ...
                        {
                            "role": "user",
                            "content": f"Predict whether the following news article is fake news (0 for fake and 1 for not fake). Please respond in JSON format, like this example: {{'fake': integer}}. Please avoid providing additional explanations. Article text:\n{row['Text']}"
                        },
                        {
                            "role": "assistant",
                            "content": f"{{'fake': {row['Label']}}}"
                        }
                    ]
                }
            )
        ...
        for index, row in df.iterrows():
            ...
            if pd.isnull(row[prediction_column]):
                ...
                if not prediction['status']:
                    ...
                    if prediction['data']['fake'] != '':  # TODO! Change it!
                        # Update the DataFrame with the evaluation result
                        # df.at[index, prediction_column] = prediction['data']['fake']  # TODO! Change it!
                        # for integers only
                        df.at[index, prediction_column] = int(prediction['data']['fake'])  # TODO! Change it!
