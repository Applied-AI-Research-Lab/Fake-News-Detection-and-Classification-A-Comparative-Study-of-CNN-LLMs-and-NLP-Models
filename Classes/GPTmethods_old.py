...
class GPTmethods:
            ...
            data.append(
                {
                    "messages": [
                        {
                            "role": "system",
                            "content": "You are an AI model tasked with predicting if a news article is spam or not. Respond with 0 for spam and 1 for not spam. Return your response in JSON format: {'spam': integer}."
                        },
                        {
                            "role": "user",
                            "content": f"Predict if the following news article is spam or not (0 for spam and 1 for not spam). Please respond with JSON format like this example: {{'spam': integer}}. Please avoid providing additional explanations. Article text:\n{row['Text']}"
                        },
                        {
                            "role": "assistant",
                            "content": f"{{'spam': {row['Label']}}}"
                        }
                    ]
                }
            )
    ...
    #                                          f"resulted in the following response: {response}"}
    def clean_response(self, response, a_field):
        # If the response is a dictionary, return it directly
        ...
        try:
            ...
            if start_index != -1 and end_index != -1:
                ...
                # Try parsing the corrected JSON string
                json_data = json.loads(json_str)
                return {"status": True, "data": json_data}
            else:
                logging.error(f"No JSON found in the response. The input '{a_field}', resulted in the "
    ...
    def predictions(self, data_set, prediction_column):
        ...
        if prediction_column not in df.columns:
            ...
            df = df.astype({prediction_column: 'Int64'})  # TODO! For non-int values omit this if
        ...
        for index, row in df.iterrows():
            ...
            if pd.isnull(row[prediction_column]):
                ...
                if not prediction['status']:
                    ...
                    if prediction['data']['spam'] != '':  # TODO! Change it!
                        # Update the DataFrame with the evaluation result
                        # df.at[index, prediction_column] = prediction['data']['spam']  # TODO! Change it!
                        # for integers only
                        df.at[index, prediction_column] = int(prediction['data']['spam'])  # TODO! Change it!

                        # Update the CSV file with the new evaluation values
                        df.to_csv(self.pre_path + data_set, index=False)
                    else:
                        logging.error(
                            f"No prediction instance was found within the data for '{row['Text']}', and the "
                            f"corresponding prediction response was: {prediction}.")  # TODO! Change it!
                        return {"status": False,
                                "data": f"No prediction instance was found within the data for '{row['Text']}', "
                                        f"and the corresponding prediction response was: {prediction}."}  # TODO! Change it!

                # break
            # Add a delay of 5 seconds (reduced for testing)

        # Change the column datatype after processing all predictions to handle 2.0 predictions
...
# GPT_BM = GPTmethods(model_id='gpt-4o')
